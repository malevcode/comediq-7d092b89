    ...
    <div className="relative" ref={filterRef}>
      <Button
        ...
        className={`flex items-center justify-center text-[11px] font-bold px-2 py-1 h-7 relative transition-all ${
          hasActiveFilters
            ? 'bg-white/16 border-0 text-white hover:bg-white/22 shadow-[0_12px_38px_rgba(2,10,30,0.24)] backdrop-blur-xl'
            : 'bg-white/10 border-0 text-white/82 hover:bg-white/16 shadow-[0_12px_38px_rgba(2,10,30,0.24)] backdrop-blur-xl'
        }`}
      >
        <span>Filter</span>
        {hasActiveFilters && (
      ...
      {showFilters && (
        <>
          ...
          <div className="fixed md:absolute left-1/2 -translate-x-1/2 md:translate-x-0 md:left-auto top-20 md:top-full md:right-0 md:mt-2 z-[9999] w-[92vw] max-w-sm md:w-72">
            <Card className="rounded-lg border-white/12 bg-[#102a53]/92 text-white shadow-[0_18px_55px_rgba(2,10,30,0.34)] backdrop-blur-xl">
              <CardContent className="p-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-sm text-white">Filter Open Mics</h3>
                  <Button
                    onClick={() => setShowFilters(false)}
                    variant="ghost"
                    size="sm"
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 items-end">
                    <div>
                      <label className="text-xs font-medium mb-1 block text-white/72">Frequency</label>
                      <select
                        value={filters.frequency || 'all'}
                        onChange={(e) => onFiltersChange({ ...filters, frequency: e.target.value as MicFrequency | 'all' })}
                        className="w-auto px-2 py-1 text-xs border border-white/12 rounded-md bg-white/10 text-white"
                      >
                    ...
                    <div>
                      <label className="text-xs font-medium mb-1 block text-white/72">City</label>
                      <select
                        value={filters.city}
                  ...
                  {/* Cost Filter */}
                  <div>
                    <label className="text-xs font-medium mb-2 block text-white/72">
                  ...
                  <div>
                    <label className="text-xs font-medium mb-1 block text-white/72">Borough</label>
                    <select
                      value={filters.borough}
                      onChange={(e) => onFiltersChange({ ...filters, borough: e.target.value })}
                      className="w-full px-2 py-1 text-xs border border-white/12 rounded-md bg-white/10 text-white"
                    >
                  ...
                  {/* Time of Day Filter */}
                  <div>
                    <label className="text-xs font-medium mb-1 block text-white/72">Time of Day</label>
                        ...
                        return (
                          <Button
                            ...
                            className={`text-xs h-7 px-1 ${
                              active
                                ? 'bg-white/18 border-white/24 text-white hover:bg-white/24'
                                : 'border-white/12 text-white/72 hover:bg-white/10'
                            }`}
                          >
                            {short}
                          </Button>
